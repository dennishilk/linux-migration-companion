...
`0.3.0-rc.1` is a website release candidate. Until a public version is explicitly tagged, only the latest reviewed commit on `main` is maintained.
...
The optional downloadable collectors run only after the user independently downloads, inspects and starts them outside the web application. Their narrower boundary is fixed: explicit read allowlists, no elevation/root, no package/driver/configuration change, no user-file access, no network, no telemetry, one create-new JSON output, and no execution of collected data. Crossing that boundary requires a new threat model and independent review.
...
- Collector-side prohibited-key self-checks plus tests that execute Linux output validation and statically audit Windows WQL property allowlists, mutating/network/elevation commands and create-new output handling.
- Linux output uses mode `0600`, `O_EXCL` and `O_NOFOLLOW` where available; Windows uses `FileMode.CreateNew`. User-selected parent directories remain a documented trust boundary.
