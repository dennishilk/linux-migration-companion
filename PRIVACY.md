...
The Alpha has no application backend, user account, analytics, advertising, telemetry, fingerprinting, or third-party font request. Advisor answers, selected software, manual hardware notes, live-test results, media progress, locale, and selected distribution are evaluated and stored in the browser as a versioned Migration Passport.
...
The key `linux-migration-companion:passport:v1` is stored in `localStorage`. It remains on that browser profile until the user resets site data, clears browser storage, or uses “Reset local Passport.” Private/incognito browsing may discard it earlier.
...
Import is local. Files are limited to 256 KiB and validated against a strict schema. Unknown keys, unknown catalog identifiers, excessive nesting, and invalid values are rejected. The file is not uploaded by this application.

## Hardware scanner decision

The proposed Windows hardware scanner is deferred. The browser cannot obtain sufficient trustworthy evidence, and the Alpha does not ship a native executable that could enumerate identifiers. Manual notes and live-test results keep the privacy and trust boundary visible.
